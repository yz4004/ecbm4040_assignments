...
def dropout_forward(x, dropout_config, mode):
    ...
    - cache: (train phase) cache a random dropout mask used in feedforward process
             (test phase) None
    """
    keep_prob = dropout_config.get("keep_prob", 0.7)  # task2 -> mlp -> here. dropout_config is a dict.   dict.get(key, default=None) return                                             # value of key. if no such key return default value, here is 0.7
    ...
    if mode == "train":
        ...
        mask =  (np.random.uniform(0,1,x.shape) < keep_prob).astype(x.dtype) # keep_prob = 0.7: 0.7 chance that neuron value being kept.
        out = x*mask # / (1.0 - keep_prob)
        cache = mask
        ...
        # mask =  (np.random.uniform(0,1,x.shape) < keep_prob).astype(x.dtype)
        # out = x*mask # / (1.0 - keep_prob)
        # cache = None 
        out = keep_prob*x
...
def bn_forward(x, gamma, beta, bn_params, mode):
    ...
    stored in bn_config to make normalization transform. (bn_config will be pass to bn_params) ???
...
#     moving_var = bn_params.get('moving_var', np.ones(D, dtype=x.dtype))
    moving_mean = bn_params.get('moving_mean', np.zeros(D))
    moving_var = bn_params.get('moving_var', np.ones(D))
