...
class SGDmomentumOptim(Optimizer):
    def __init__(self, model, momentum=0.5):
        ...
        velocitys = dict()
        for k, v in model.params.items():# key weight_name: value: W
    ...
    def step(self, model, learning_rate):
        ...
        # model.params, model.grads are dict! storing each layers'
...
#         Runoob www.runoob.com
        ...
        # update model (update model parameters)
        
        self.velocitys = velocitys
        model.params = params
        # model.grads = grads  # where do we update grads? 
...
class AdaGradOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        for k in grads:
            ...
            cache[k] += grads[k] ** 2 # element-wise
            update_grad = - learning_rate / np.sqrt(cache[k] + eps) * grads[k]
            params[k] += update_grad
            
            # ada_learning_rate = learning_rate / np.sqrt(eps + cache[k])
            # params[k] += ada_learning_rate *grads[k]
            # cache[k] += np.sum(grads[k] ** 2)
...
class NadamOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        for k in grads:
            ...
            velocitys_hat = velocitys[k]/(1 - beta2 ** t)
            momentum_hat = momentums[k]/(1 - beta1 ** t)
            
            update = - learning_rate/(np.sqrt(velocitys_hat) + eps )*(beta1*momentum_hat + (1-beta1)*grads[k]/(1-beta1**t))
